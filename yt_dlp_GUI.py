    ...
    if flag.get():
        if len(folder.get()) == 0:
            if rename.get():
                ...
                os.system('yt-dlp -f mp4 -o "' + name.get() + '.mp4" ' + link)
            else:
                # print("MP4 Same dir same name")
                os.system('yt-dlp -f mp4 -o "%(title)s.%(ext)s" ' + link)
        else:
            if rename.get():
                ...
                os.system(
                    'yt-dlp -f mp4 -o "' + folder.get() + '/%(title)s.%(ext)s" ' + link
                )
    else:
        if mdoff.get():
        ...
        if len(folder.get()) == 0:
            if rename.get():
                ...
                if mdoff.get():
                    audiofile = eyed3.load(os.getcwd() + "\\" + name.get() + ".mp3")
                ...
                os.system(
                    'yt-dlp -o "%(title)s.%(ext)s" --extract-audio --audio-format mp3 '
                    + link
                )
                if mdoff.get():
            ...
            if rename.get():
                ...
                if mdoff.get():
                    audiofile = eyed3.load(folder.get() + "\\" + name.get() + ".mp3")
                    if edit_metadata.get():
                        audiofile.tag.artist = artist.get()
                        audiofile.tag.album = album.get()
                    else:
...
Checkbutton(
    gui,
    text="Specify a name for your file",
    variable=rename,
    onvalue="True",
    offvalue="False",
    height=1,
...
conv = Button(gui, text="Convert", width=10, height=1, command=down).place(x=250, y=150)

meta_check = Checkbutton(
    gui,
    text="Specify your own metadata for the *.mp3 file",
    variable=edit_metadata,
    onvalue="True",
    offvalue="False",
    height=1,
    width=50,
...
meta_check2 = Checkbutton(
    gui,
    text="Write metadata to this song",
    variable=mdoff,
    onvalue="True",
    offvalue="False",
    height=1,
